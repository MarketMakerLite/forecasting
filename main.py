...
def initializer(symbol):
    ...
    num_data_points = 252
    one_year_ago = (datetime.datetime.utcnow().date() - datetime.timedelta(days=num_data_points * 1.3)).strftime("%Y-%m-%d")
...
def arima(symbol, df, period, date_range):
    ...
    results_df = results_df[['uticker', 'tdate', 'arima_low', 'arima_forecast', 'arima_high', 'arima_order']]
    return results_df


def garch_model(df, period, date_range):
    df['tdate'] = pd.to_datetime(df['tdate']).sort_values(ascending=True)
    df.set_index(df['tdate'], inplace=True)
    ...
    forecasts = res.forecast(horizon=period, method='analytic', reindex=False)

    f_mean = forecasts.mean.iloc[-3:].iloc[0].reset_index().iloc[:, 1]
    f_vol = np.sqrt(forecasts.variance.iloc[-3:]).iloc[0].reset_index().iloc[:, 1]
    f_res = np.sqrt(forecasts.residual_variance.iloc[-3:]).iloc[0].reset_index().iloc[:, 1]

    h_vol = statistics.stdev(returns)
    h_mean = statistics.mean(returns)

    temp_df = pd.DataFrame()
    temp_df['garch_mean'] = f_mean
    temp_df['garch_vol'] = f_vol
    # temp_df['garch_res'] = f_res
    temp_df['h_vol'] = h_vol
    temp_df['h_mean'] = h_mean
